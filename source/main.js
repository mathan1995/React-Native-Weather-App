...
class MainPage extends Component {
  ...
  reloadView(selectCity) {
    Utilies.readCityList(citys => {
      ...
      this.refs.rootScroll.scrollTo({
        x: Utilies.dimensions.width * index,
        y: 0,
        animated: false
      });
    });
  ...
  createWeatherViews() {
    ...
    for (var i = 0; i < cityList.length; i++) {
      views.push(<WeatherView city={cityList[i]} key={i} />);
    }
    return views;
  }
  render() {
    return (
      <View style={{ flex: 1, backgroundColor: Utilies.commonBgColor }}>
        ...
        <BottomToolBar
          onPressRight={() => this.toCityList()}
          pageCount={
            this.state.cityList == null ? 0 : this.state.cityList.length
          }
...
class Main extends Component {
  ...
  configureScene(route, routeStack) {
    if (route.type == "Bottom") {
      return Navigator.SceneConfigs.FloatFromBottom; //  Bottom pop up
    }
    return Navigator.SceneConfigs.PushFromRight; // Pop right
  }
